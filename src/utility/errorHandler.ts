...
export type errorHandlerProps<T = any> = {
  endpoint: string;
  successHandler: (response: AxiosResponse<T>) => Generator<any, void, any>;
  failHandler: (errorMessage: string) => Generator<any, void, any>;
  payload?: any;
  apiType: string;
  failHandlerType: string;
};

export default function* errorHandler(props: errorHandlerProps) {
  const { endpoint, successHandler, failHandler, apiType, failHandlerType } =
    props;
  if (apiType.trim() === "") {
